...
### 4.3. `dt_chuong_trinh` — Chương trình đào tạo (CTĐT)
- `ma_chuong_trinh` UNIQUE, `ten_chuong_trinh`, `thoi_luong`, `khoa_phong_id` (FK → `dm_khoa_phong`, khoa phụ trách), `ngay_bat_dau`, `ngay_ket_thuc`, `so_luong_toi_da`, `dia_diem`, `giao_vien_id`.
- **Tồn tại độc lập**, KHÔNG bắt buộc thuộc khóa học (quan hệ N:N qua `dt_khoa_hoc_chuong_trinh`).
- (Trước đây là `dt_lop_hoc` với cột `ma_lop`/`ten_lop`/`khoa_hoc_id` — đã đổi 2026-06-12.)

### 4.4. `dt_khoa_hoc_chuong_trinh` — N:N Khóa học × CTĐT (bridge)
- `id` PK, `(khoa_hoc_id, chuong_trinh_id, da_xoa)` UNIQUE.
- **`id` là "ngữ cảnh học vụ"** (học CTĐT này trong khóa này). Mọi bảng học vụ trỏ vào đây qua cột `khoa_hoc_chuong_trinh_id`.

### 4.5. `dt_chuong_trinh_mon_hoc` — N:N CTĐT × môn học
- `(chuong_trinh_id, mon_hoc_id, da_xoa)` UNIQUE + `thu_tu`, `bat_buoc`. Môn gắn theo CTĐT (dùng chung mọi khóa).
- (Trước đây là `dt_lop_hoc_mon_hoc`. Lưu ý: code cũ tham chiếu nhầm bảng `dt_khoa_hoc_mon_hoc` không tồn tại — đã sửa.)

### 4.5b. `dt_hoc_vien_lop` — N:N học viên × ngữ cảnh (khóa+CTĐT)
- `(khoa_hoc_chuong_trinh_id, hoc_vien_id, da_xoa)` UNIQUE. Cột FK trỏ `dt_khoa_hoc_chuong_trinh.id`.
- `ngay_ghi_danh`, `diem_tong_ket`, `xep_loai`. Class PHP vẫn tên `DT_HocVienLop`.

> **Lưu ý chung (2026-06-12):** các bảng học vụ dưới đây đã đổi cột `lop_hoc_id` → **`khoa_hoc_chuong_trinh_id`** (FK → `dt_khoa_hoc_chuong_trinh.id`, ngữ cảnh khóa+CTĐT). Field PHP trong DTO/PUBLIC giữ tên `lop_hoc_id` (alias) để tương thích GUI; SELECT alias `ct.ma_chuong_trinh AS ma_lop`, `ct.ten_chuong_trinh AS ten_lop`.

### 4.6. `dt_lich_hoc` — Buổi học
- `khoa_hoc_chuong_trinh_id`, `buoi_thu`, `ngay_hoc`, `gio_bat_dau`, `gio_ket_thuc`, `phong_hoc`.
- `giang_vien_id`, `giang_vien_ngoai` (text) — giảng viên không có trong hệ thống.
...
- `(giang_vien_id, khoa_hoc_chuong_trinh_id, mon_hoc_id, vai_tro)` UNIQUE.
...
- `hoc_vien_id`, `khoa_hoc_chuong_trinh_id`, `so_chung_chi`, `ngay_cap`, `ngay_het_han`.
...
- `khoa_hoc_id` (khóa đăng ký) + `khoa_hoc_chuong_trinh_id` (ngữ cảnh khi duyệt).
- Khi duyệt → tự tạo `dm_hoc_vien` + `dt_hoc_vien_lop` (set `hoc_vien_id`, `khoa_hoc_chuong_trinh_id`).

### 4.13. `dt_ho_so_hoc_vien` — Hồ sơ đính kèm của HV
- `loai_ho_so` (CCCD, Bằng cấp, Chứng chỉ…), `duong_dan` file.

### 4.14. `dt_tai_lieu` — Tài liệu khoá/CTĐT/môn
- `loai_tai_lieu`: 1=Giáo trình, 2=Bài giảng, …
- Có thể gắn với `khoa_hoc_id` (khóa trực tiếp), `khoa_hoc_chuong_trinh_id` (ngữ cảnh khóa+CTĐT), hoặc `mon_hoc_id`.
...
### Đào tạo (chuỗi học vụ) — mô hình CTĐT (2026-06-12)
```
dt_khoa_hoc ─┐
             ├─< dt_khoa_hoc_chuong_trinh (N:N, id = ngữ cảnh) >─┬─ dt_chuong_trinh ─< dt_chuong_trinh_mon_hoc >─ dt_mon_hoc
dt_chuong_trinh ─┘                                              │
                                                                ▼ (khoa_hoc_chuong_trinh_id)
   dt_hoc_vien_lop >── dm_hoc_vien
   dt_lich_hoc ──< dt_diem_danh
   dt_phan_cong_giang_vien >── dm_giang_vien
   dt_bai_kiem_tra, dt_chung_chi, dt_tai_lieu, dt_dang_ky_khoa_hoc
