...
require_once __DIR__ . '/../PUBLIC/Entities/DM_DoiTuongHocVien_DTO.php';

class DM_DoiTuongHocVien_DAL
{
    private static function selectSql(): string
    {
        return "SELECT dt.*,
                ...
                FROM DM_DOI_TUONG_HOC_VIEN dt
                LEFT JOIN DM_NGUOI_DUNG u1 ON u1.id = dt.nguoi_tao
                LEFT JOIN DM_NGUOI_DUNG u2 ON u2.id = dt.nguoi_cap_nhat";
    }

    public static function insert(DM_DoiTuongHocVien_PUBLIC $e): int
    {
        $sql = "INSERT INTO DM_DOI_TUONG_HOC_VIEN (ma_doi_tuong, ten_doi_tuong, mo_ta, thu_tu, trang_thai,
                                                   ngay_tao, ngay_cap_nhat, nguoi_tao, nguoi_cap_nhat, da_xoa)
                VALUES (:ma, :ten, :mt, :tt2, :tt, NOW(), NOW(), :u1, :u2, 0)";
        $stmt = Database::getConnection()->prepare($sql);
        $stmt->execute([
            ':ma' => $e->ma_doi_tuong, ':ten' => $e->ten_doi_tuong, ':mt' => $e->mo_ta,
    ...
    public static function update(DM_DoiTuongHocVien_PUBLIC $e): int
    {
        $sql = "UPDATE DM_DOI_TUONG_HOC_VIEN SET ma_doi_tuong=:ma, ten_doi_tuong=:ten, mo_ta=:mt,
    ...
    {
        $stmt = Database::getConnection()->prepare("UPDATE DM_DOI_TUONG_HOC_VIEN SET da_xoa=1, ngay_cap_nhat=NOW(), nguoi_cap_nhat=:u WHERE id=:id");
    ...
    {
        $stmt = Database::getConnection()->prepare("UPDATE DM_DOI_TUONG_HOC_VIEN SET da_xoa=0, ngay_cap_nhat=NOW(), nguoi_cap_nhat=:u WHERE id=:id");
        $stmt->execute([':u' => $u, ':id' => $id]);
        return $stmt->rowCount();
    }
    ...
    {
        $stmt = Database::getConnection()->prepare("DELETE FROM DM_DOI_TUONG_HOC_VIEN WHERE id=:id");
        $stmt->execute([':id' => $id]);
        return $stmt->rowCount();
    }
    ...
    public static function getById(int $id): ?DM_DoiTuongHocVien_DTO
    {
        ...
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
        return $row ? Database::hydrate($row, 'DM_DoiTuongHocVien_DTO') : null;
    }
    ...
    public static function getPaged(int $page, int $pageSize, string $search = '', int $daXoa = 0): array
    {
        ...
        $sql = self::selectSql() . $where . " ORDER BY dt.thu_tu ASC, dt.id ASC LIMIT {$pageSize} OFFSET {$offset}";
        $stmt = Database::getConnection()->prepare($sql);
        $stmt->execute($params);
        return [
            'data' => $stmt->fetchAll(),
    ...
    {
        $stmt = Database::getConnection()->prepare("SELECT COUNT(*) FROM DM_DOI_TUONG_HOC_VIEN WHERE ma_doi_tuong=:m AND da_xoa=0 AND id<>:id");
    ...
    {
        $key = 'dm_doi_tuong_hoc_vien:combo';
        $cached = MemcachedHelper::get($key);
        if ($cached !== null) return $cached;
        $stmt = Database::getConnection()->query("SELECT id, ma_doi_tuong, ten_doi_tuong FROM DM_DOI_TUONG_HOC_VIEN WHERE da_xoa=0 AND trang_thai=1 ORDER BY thu_tu ASC, ten_doi_tuong ASC");
        $data = $stmt->fetchAll();
