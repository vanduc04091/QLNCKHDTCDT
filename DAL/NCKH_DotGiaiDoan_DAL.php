...
require_once __DIR__ . '/../PUBLIC/Entities/NCKH_DotGiaiDoan_DTO.php';

class NCKH_DotGiaiDoan_DAL
{
    private static function selectSql(): string
    {
        return "SELECT gd.*, d.ten_dot
                FROM NCKH_DOT_GIAI_DOAN gd
                LEFT JOIN NCKH_DOT_DANG_KY d ON d.id = gd.dot_id";
    }

    public static function insert(NCKH_DotGiaiDoan_PUBLIC $e): int
    {
        $sql = "INSERT INTO NCKH_DOT_GIAI_DOAN
    ...
    public static function update(NCKH_DotGiaiDoan_PUBLIC $e): int
    {
        $sql = "UPDATE NCKH_DOT_GIAI_DOAN SET
    ...
    {
        $stmt = Database::getConnection()->prepare("UPDATE NCKH_DOT_GIAI_DOAN SET da_xoa=1, ngay_cap_nhat=NOW(), nguoi_cap_nhat=:u WHERE id=:id");
    ...
    public static function getById(int $id): ?NCKH_DotGiaiDoan_DTO
    {
        ...
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
        return $row ? Database::hydrate($row, 'NCKH_DotGiaiDoan_DTO') : null;
    }
    ...
    /**
     * Lấy giai đoạn ĐANG MỞ theo hành vi (Submit|Edit|Review) cho 1 đợt.
     * Trả null nếu không có giai đoạn nào đang mở.
     */
    public static function getActivePhase(int $dotId, string $hanhVi): ?array
    {
        $sql = "SELECT * FROM NCKH_DOT_GIAI_DOAN
