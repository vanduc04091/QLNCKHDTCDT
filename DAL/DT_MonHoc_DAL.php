...
class DT_MonHoc_DAL
{
    private static function selectSql(): string
    {
        return "SELECT mh.*,
                       ...
                       (SELECT COUNT(*) FROM DT_KHOA_HOC_MON_HOC km WHERE km.mon_hoc_id = mh.id AND km.da_xoa = 0) AS so_khoa_hoc_su_dung
                FROM DT_MON_HOC mh
    ...
    public static function getPaged(int $page, int $pageSize, string $search = '', int $daXoa = 0, int $trangThai = -1): array
    {
        ...
        $params = [':dx' => $daXoa];
        if ($search !== '') {
            $where .= " AND (mh.ma_mon_hoc LIKE :s OR mh.ten_mon_hoc LIKE :s) ";
            $params[':s'] = "%{$search}%";
        }
        if ($trangThai >= 0) {
    ...
    public static function getStats(): array
    {
        ...
        $inKhoa = (int)$pdo->query("SELECT COUNT(DISTINCT mon_hoc_id) FROM DT_KHOA_HOC_MON_HOC WHERE da_xoa=0")->fetchColumn();
    ...
    public static function getCombo(): array
    {
        ...
        $stmt = Database::getConnection()->query(
            "SELECT id, ma_mon_hoc, ten_mon_hoc, so_tiet_ly_thuyet, so_tiet_thuc_hanh, tong_so_tiet, so_tin_chi
             FROM DT_MON_HOC WHERE da_xoa=0 AND trang_thai=1 ORDER BY ten_mon_hoc"
        );
        $data = $stmt->fetchAll();
    ...
    public static function isUsedInKhoaHoc(int $monHocId): int
    {
        $stmt = Database::getConnection()->prepare(
            "SELECT COUNT(*) FROM DT_KHOA_HOC_MON_HOC WHERE mon_hoc_id=:id AND da_xoa=0"
        );
