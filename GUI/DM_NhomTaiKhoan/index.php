...
Helper::requireLogin();
if (!PhanQuyenHelper::hasQuyen('DM_NhomTaiKhoan', PhanQuyenHelper::QUYEN_XEM)) {
...
<div class="card">
    <div class="toolbar">
        <div class="left">
            <input type="text" id="search" class="form-control" placeholder="Tìm mã, tên nhóm..." style="max-width:340px">
        ...
        <div class="right">
            <?php if ($canAdd): ?>
                <button type="button" class="btn btn-primary" onclick="openCreate()">+ Thêm nhóm</button>
            <?php endif; ?>
        </div>
    </div>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="ma_nhom" id="f_ma_nhom" class="form-control" required maxlength="50">
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="ten_nhom" id="f_ten_nhom" class="form-control" required maxlength="200">
                </div>
                <div class="form-group">
                    <label>Mô tả</label>
...
<script>
var URL = APP_BASE + 'GUI/DM_NhomTaiKhoan/ajax_handler.php';
    ...
    rows.forEach(function (r) {
        ...
        $tb.append(
            '<tr>' +
                ...
                '<td><strong>' + APP.escape(r.ma_nhom) + '</strong></td>' +
                '<td>' + APP.escape(r.ten_nhom) + '</td>' +
                '<td>' + APP.escape(r.mo_ta || '-') + '</td>' +
                '<td class="text-center">' + (r.so_nguoi_dung || 0) + '</td>' +
...
function openEdit(id) {
    APP.ajax(URL, {action: 'getById', id: id}).done(function (res) {
        ...
        $('#f_id').val(e.id);
        $('#f_ma_nhom').val(e.ma_nhom);
        $('#f_ten_nhom').val(e.ten_nhom);
        $('#f_mo_ta').val(e.mo_ta || '');
...
function deleteItem(id) {
    APP.confirm('Xóa VĨNH VIỄN nhóm này? Các phân quyền của nhóm cũng sẽ bị xóa!', function () {
