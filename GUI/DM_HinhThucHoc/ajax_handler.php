...
$MODULE = DM_HinhThucHoc_BUS::MODULE_KEY;
...
try {
    switch ($action) {
        case 'getPaged':
            ...
            $res = DM_HinhThucHoc_BUS::getPaged($page, $size, $search, $daXoa);
            ResponseHelper::paged($res['data'], $page, $size, $res['totalRecords']);
            break;

        case 'getById':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_XEM);
            $e = DM_HinhThucHoc_BUS::getById(Helper::postInt('id'));
        ...
        case 'insert':
            ...
            $e = new DM_HinhThucHoc_PUBLIC();
            $e->ma_hinh_thuc = Helper::postStr('ma_hinh_thuc');
            $e->ten_hinh_thuc = Helper::postStr('ten_hinh_thuc');
            $e->mo_ta = Helper::postStr('mo_ta');
            $e->thu_tu = Helper::postInt('thu_tu', 0);
            $e->trang_thai = Helper::postInt('trang_thai', 1);
            $e->nguoi_tao = $u;
            $res = DM_HinhThucHoc_BUS::insert($e);
        ...
        case 'update':
            ...
            $e->trang_thai = Helper::postInt('trang_thai', 1);
            $e->nguoi_cap_nhat = $u;
            $res = DM_HinhThucHoc_BUS::update($e);
        ...
        case 'trash':
            ...
            $res = DM_HinhThucHoc_BUS::trash(Helper::postInt('id'), $u);
            $res['success'] ? ResponseHelper::success($res['message']) : ResponseHelper::error($res['message']);
            break;

        case 'restore':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_SUA);
            $res = DM_HinhThucHoc_BUS::restore(Helper::postInt('id'), $u);
            $res['success'] ? ResponseHelper::success($res['message']) : ResponseHelper::error($res['message']);
            break;

        case 'delete':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_XOA);
            $res = DM_HinhThucHoc_BUS::delete(Helper::postInt('id'));
            $res['success'] ? ResponseHelper::success($res['message']) : ResponseHelper::error($res['message']);
            break;
