...
require_once __DIR__ . '/../../BUS/NCKH_DotGiaiDoan_BUS.php';

Helper::requireAjaxCsrf();

$action = Helper::post('action', '');
$u = SessionHelper::userId();
$MODULE = NCKH_DotDangKy_BUS::MODULE_KEY;
...
try {
    switch ($action) {
        ...
        case 'getPaged':
            ...
            $res = NCKH_DotDangKy_BUS::getPaged($page, $size, array_filter($opts, function ($v) { return $v !== '' && $v !== null; }));
            ResponseHelper::paged($res['data'], $page, $size, $res['totalRecords']);
            break;

        case 'getById':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_XEM);
            $e = NCKH_DotDangKy_BUS::getById(Helper::postInt('id'));
        ...
        case 'getCombo':
            ...
            ResponseHelper::success('OK', NCKH_DotDangKy_BUS::getCombo($only));
            break;

        case 'insert':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_THEM);
            $e = new NCKH_DotDangKy_PUBLIC();
            ...
            $e->nguoi_tao = $u;
            $res = NCKH_DotDangKy_BUS::insert($e);
        ...
        case 'update':
            ...
            $e->trang_thai = Helper::postInt('trang_thai', 1);
            $e->nguoi_cap_nhat = $u;
            $res = NCKH_DotDangKy_BUS::update($e);
        ...
        case 'trash':
            ...
            $res = NCKH_DotDangKy_BUS::trash(Helper::postInt('id'), $u);
            $res['success'] ? ResponseHelper::success($res['message']) : ResponseHelper::error($res['message']);
            break;
        ...
        case 'getPhases':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_XEM);
            ResponseHelper::success('OK', NCKH_DotGiaiDoan_BUS::getByDot(Helper::postInt('dot_id')));
            break;

        case 'getPhaseById':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_XEM);
            $gd = NCKH_DotGiaiDoan_BUS::getById(Helper::postInt('id'));
        ...
        case 'insertPhase':
            ...
            $e->nguoi_tao = $u;
            $res = NCKH_DotGiaiDoan_BUS::insert($e);
            $res['success'] ? ResponseHelper::success($res['message'], $res['data'] ?? null) : ResponseHelper::error($res['message']);
            break;
        ...
        case 'updatePhase':
            ...
            $e->ghi_chu = Helper::postStr('ghi_chu') ?: null;
            $e->nguoi_cap_nhat = $u;
            $res = NCKH_DotGiaiDoan_BUS::update($e);
            $res['success'] ? ResponseHelper::success($res['message']) : ResponseHelper::error($res['message']);
            break;

        case 'trashPhase':
            PhanQuyenHelper::requireQuyen($MODULE, PhanQuyenHelper::QUYEN_XOA);
            $res = NCKH_DotGiaiDoan_BUS::trash(Helper::postInt('id'), $u);
            $res['success'] ? ResponseHelper::success($res['message']) : ResponseHelper::error($res['message']);
            break;
