...
Helper::requireLogin();
if (!PhanQuyenHelper::hasQuyen('DM_NCKH_TheLoai', PhanQuyenHelper::QUYEN_XEM)) {
    echo 'Bạn không có quyền truy cập chức năng này.'; exit;
}

$canAdd = PhanQuyenHelper::hasQuyen('DM_NCKH_TheLoai', PhanQuyenHelper::QUYEN_THEM);
$canEdit = PhanQuyenHelper::hasQuyen('DM_NCKH_TheLoai', PhanQuyenHelper::QUYEN_SUA);
$canDel = PhanQuyenHelper::hasQuyen('DM_NCKH_TheLoai', PhanQuyenHelper::QUYEN_XOA);

$pageTitle = 'Thể loại NCKH';
$activeMenu = 'DM_NCKH_TheLoai';
...
<div class="breadcrumb">
    ...
    <span class="sep">›</span> <span>Thể loại</span>
</div>
...
<div class="modal-backdrop" id="modalForm">
    <div class="modal">
        <div class="modal-header">
            <h3 id="modalTitle">Thêm thể loại</h3>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="ma_the_loai" id="f_ma" class="form-control" required maxlength="20">
                    </div>
                    <div class="form-group">
                        <label>Thứ tự</label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="ten_the_loai" id="f_ten" class="form-control" required maxlength="150">
                </div>
                <div class="form-group">
                    <label>Mô tả</label>
...
<script>
var URL = APP_BASE + 'GUI/DM_NCKH_TheLoai/ajax_handler.php';
...
function load() {
    ...
    APP.ajax(URL, {action:'getPaged', page:state.page, pageSize:state.pageSize, search:state.search, da_xoa:state.daXoa}).done(function (res) {
...
function renderRows(rows) {
    var $tb = $('#tbody').empty();
    if (!rows.length) { $tb.append('<tr><td colspan="7"><div class="empty-state">Không có dữ liệu</div></td></tr>'); return; }
    var stt = (state.page - 1) * state.pageSize;
    rows.forEach(function (r) {
        stt++;
        var tt = r.trang_thai == 1 ? '<span class="badge badge-success">Hoạt động</span>' : '<span class="badge badge-danger">Khóa</span>';
        var actions = '';
        if (state.daXoa == 0) {
        ...
        $tb.append(
            '<tr>' +
                ...
                '<td><strong>' + APP.escape(r.ma_the_loai) + '</strong></td>' +
...
function openCreate() { $('#modalTitle').text('Thêm thể loại'); $('#formMain')[0].reset(); $('#f_id').val(''); $('#modalForm').addClass('open'); }
function openEdit(id) {
    APP.ajax(URL, {action:'getById', id:id}).done(function (res) {
        if (!res.success) { APP.toast(res.message,'error'); return; }
        var e = res.data;
        $('#modalTitle').text('Sửa thể loại');
        $('#f_id').val(e.id); $('#f_ma').val(e.ma_the_loai); $('#f_ten').val(e.ten_the_loai);
        $('#f_mo_ta').val(e.mo_ta || ''); $('#f_thu_tu').val(e.thu_tu || 0); $('#f_trang_thai').val(e.trang_thai);
...
function restoreItem(id) { APP.ajax(URL, {action:'restore', id:id}).done(function (res) { res.success ? (APP.toast(res.message,'success'), load()) : APP.toast(res.message,'error'); }); }
function deleteItem(id) {
    APP.confirm('Xóa VĨNH VIỄN?', function () {
        APP.ajax(URL, {action:'delete', id:id}).done(function (res) { res.success ? (APP.toast(res.message,'success'), load()) : APP.toast(res.message,'error'); });
