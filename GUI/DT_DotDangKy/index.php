...
Helper::requireLogin();
if (!PhanQuyenHelper::hasQuyen('DT_DotDangKy', PhanQuyenHelper::QUYEN_XEM)) {
    echo 'Bạn không có quyền truy cập chức năng này.'; exit;
}
$canAdd  = PhanQuyenHelper::hasQuyen('DT_DotDangKy', PhanQuyenHelper::QUYEN_THEM);
$canEdit = PhanQuyenHelper::hasQuyen('DT_DotDangKy', PhanQuyenHelper::QUYEN_SUA);
$canDel  = PhanQuyenHelper::hasQuyen('DT_DotDangKy', PhanQuyenHelper::QUYEN_XOA);

$pageTitle = 'Đợt đăng ký khóa học';
$activeMenu = 'DT_DotDangKy';
...
<div class="breadcrumb">
    ...
    <span class="sep">›</span> <span>Đợt đăng ký khóa học</span>
</div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th style="width:90px" class="text-center">Khóa học</th>
        ...
        <form id="formPhase">
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="ten_giai_doan" id="p_ten" class="form-control" required maxlength="255" placeholder="VD: Mở đăng ký khóa K3">
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>Hành vi <span class="required">*</span></label>
                        <select name="hanh_vi" id="p_hv" class="form-select" required>
                            <option value="Submit">Đăng ký khóa học</option>
                            <option value="Review">Duyệt đăng ký</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Thứ tự</label>
...
<script>
var URL = APP_BASE + 'GUI/DT_DotDangKy/ajax_handler.php';
...
function renderRows(rows) {
    ...
    rows.forEach(function (r) {
        ...
        var actions = '<button class="btn btn-sm" title="Xem giai đoạn" onclick="openDrawer(' + r.id + ')">' + ICON_EYE + '</button>';
...
function openEdit(id) {
    APP.ajax(URL, {action:'getById', id:id}).done(function (res) {
        ...
        $('#f_id').val(e.id); $('#f_ten').val(e.ten_dot); $('#f_nam').val(e.nam);
        $('#f_tn').val(e.tu_ngay); $('#f_dn').val(e.den_ngay);
        $('#f_mt').val(e.mo_ta || ''); $('#f_tt').val(e.trang_thai);
        $('#modalDot').addClass('open');
    });
}
...
function renderPhases(rows) {
    ...
    if (!rows.length) { $list.html('<div class="empty-state">Chưa có giai đoạn nào. Thêm các giai đoạn để mở/khóa thao tác đăng ký.</div>'); return; }
    var now = new Date();
    rows.forEach(function (r) {
...
function openPhase(id) {
    ...
    if (id) {
        ...
        APP.ajax(URL, {action:'getPhaseById', id:id}).done(function (res) {
            if (!res.success) { APP.toast(res.message,'error'); return; }
            var e = res.data;
            $('#p_id').val(e.id); $('#p_ten').val(e.ten_giai_doan);
            $('#p_hv').val(e.hanh_vi); $('#p_thu_tu').val(e.thu_tu || 0);
        ...
        $('#phaseTitle').text('Thêm giai đoạn'); $('#p_id').val('');
        $('#modalPhase').addClass('open');
    }
}
