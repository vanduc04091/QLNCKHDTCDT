...
Helper::requireLogin();
if (!PhanQuyenHelper::hasQuyen('DM_DoiTuongHocVien', PhanQuyenHelper::QUYEN_XEM)) {
    echo 'Bạn không có quyền truy cập chức năng này.'; exit;
}

$canAdd = PhanQuyenHelper::hasQuyen('DM_DoiTuongHocVien', PhanQuyenHelper::QUYEN_THEM);
$canEdit = PhanQuyenHelper::hasQuyen('DM_DoiTuongHocVien', PhanQuyenHelper::QUYEN_SUA);
$canDel = PhanQuyenHelper::hasQuyen('DM_DoiTuongHocVien', PhanQuyenHelper::QUYEN_XOA);

$pageTitle = 'Quản lý đối tượng học viên';
$activeMenu = 'DM_DoiTuongHocVien';
...
<div class="breadcrumb">
    ...
    <span class="sep">›</span> <span>Đối tượng học viên</span>
</div>
...
<div class="modal-backdrop" id="modalForm">
    <div class="modal">
        <div class="modal-header">
            <h3 id="modalTitle">Thêm đối tượng học viên</h3>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="ma_doi_tuong" id="f_ma" class="form-control" required maxlength="20">
                    </div>
                    <div class="form-group">
                        <label>Thứ tự</label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="ten_doi_tuong" id="f_ten" class="form-control" required maxlength="200">
                </div>
                <div class="form-group">
                    <label>Mô tả</label>
...
<script>
var URL = APP_BASE + 'GUI/DM_DoiTuongHocVien/ajax_handler.php';
var CAN_EDIT = <?= $canEdit?'true':'false' ?>;
var CAN_DEL = <?= $canDel?'true':'false' ?>;
var state = { page: 1, pageSize: 20, search: '', daXoa: 0 };
function exportExcel(){ var p=new URLSearchParams({search:state.search||'',da_xoa:state.daXoa||0}); window.location=APP_BASE+'GUI/DM_DoiTuongHocVien/export.php?'+p.toString(); }
    ...
    rows.forEach(function (r) {
        ...
        $tb.append(
            '<tr>' +
                ...
                '<td><strong>' + APP.escape(r.ma_doi_tuong) + '</strong></td>' +
...
function openCreate() {
    $('#modalTitle').text('Thêm đối tượng học viên');
...
function openEdit(id) {
    APP.ajax(URL, {action: 'getById', id: id}).done(function (res) {
        if (!res.success) { APP.toast(res.message, 'error'); return; }
        var e = res.data;
        $('#modalTitle').text('Sửa đối tượng học viên');
        $('#f_id').val(e.id);
        $('#f_ma').val(e.ma_doi_tuong);
        $('#f_ten').val(e.ten_doi_tuong);
        $('#f_mo_ta').val(e.mo_ta || '');
