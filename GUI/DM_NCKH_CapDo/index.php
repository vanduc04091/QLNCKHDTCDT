...
Helper::requireLogin();
if (!PhanQuyenHelper::hasQuyen('DM_NCKH_CapDo', PhanQuyenHelper::QUYEN_XEM)) {
    echo 'Bạn không có quyền truy cập chức năng này.'; exit;
}

$canAdd = PhanQuyenHelper::hasQuyen('DM_NCKH_CapDo', PhanQuyenHelper::QUYEN_THEM);
$canEdit = PhanQuyenHelper::hasQuyen('DM_NCKH_CapDo', PhanQuyenHelper::QUYEN_SUA);
$canDel = PhanQuyenHelper::hasQuyen('DM_NCKH_CapDo', PhanQuyenHelper::QUYEN_XOA);

$pageTitle = 'Cấp độ NCKH';
$activeMenu = 'DM_NCKH_CapDo';
...
<div class="breadcrumb">
    ...
    <span class="sep">›</span> <span>Cấp độ</span>
</div>
...
<div class="modal-backdrop" id="modalForm">
    <div class="modal">
        <div class="modal-header">
            <h3 id="modalTitle">Thêm cấp độ</h3>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="ma_cap_do" id="f_ma" class="form-control" required maxlength="20">
                    </div>
                    <div class="form-group">
                        <label>Thứ tự</label>
                ...
                <div class="form-group">
                    <label>Tên cấp độ <span class="required">*</span></label>
                    <input type="text" name="ten_cap_do" id="f_ten" class="form-control" required maxlength="100">
                </div>
                <div class="form-group">
                    <label>Mô tả</label>
...
<script>
var URL = APP_BASE + 'GUI/DM_NCKH_CapDo/ajax_handler.php';
...
function renderRows(rows) {
    var $tb = $('#tbody').empty();
    if (!rows.length) {
        $tb.append('<tr><td colspan="7"><div class="empty-state">Không có dữ liệu</div></td></tr>'); return;
    }
    ...
    rows.forEach(function (r) {
        ...
        $tb.append(
            '<tr>' +
                ...
                '<td><strong>' + APP.escape(r.ma_cap_do) + '</strong></td>' +
...
function openEdit(id) {
    APP.ajax(URL, {action:'getById', id:id}).done(function (res) {
        ...
        $('#f_id').val(e.id); $('#f_ma').val(e.ma_cap_do); $('#f_ten').val(e.ten_cap_do);
...
function restoreItem(id) {
    APP.ajax(URL, {action:'restore', id:id}).done(function (res) { res.success ? (APP.toast(res.message,'success'), load()) : APP.toast(res.message,'error'); });
}
