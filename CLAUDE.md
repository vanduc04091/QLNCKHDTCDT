...
### 2026-07-16 — Nạp lại danh sách nhân viên (người hành nghề) + Import Excel
- ✅ **Thêm cột CCHN** cho `dm_nhan_vien` (SQL user tự chạy `docs/sql_nhan_vien_hanh_nghe.sql`): `pham_vi_hanh_nghe`, `so_cchn`, `ngay_cap_cchn`, `qd_bo_sung_pham_vi`, `dieu_chinh_pham_vi`, `ngay_dieu_chinh`, `chuyen_khoa_cap_nhat`, `khoa_phong_text` (lưu tên khoa gốc khi chưa map được). File SQL còn phần **wipe** (TRUNCATE `dt_cme_ghi_nhan`, gỡ `nhan_vien_id` ở `dm_giang_vien`/`dm_hoc_vien`, TRUNCATE `dm_nhan_vien`) — chạy sau khi backup.
- ✅ **`DM_NhanVien_BUS::importExcel($path,$userId)`** — đọc mẫu "Danh sách người hành nghề toàn viện" (13 cột) bằng `ExcelHelper::readRows`. Map khoa theo tên chuẩn hóa (bỏ "Khoa/Phòng/TT", dấu, khoảng trắng — 2 chỉ mục thường + bỏ hết space); không map được → vẫn thêm NV, giữ `khoa_phong_text`. Trùng mã (file/DB) → bỏ qua. Mỗi dòng transaction riêng. Parse ngày dd/mm/yyyy. Thử thực tế: 1435 NV, 1311 map được khoa, 124 giữ text (khoa DB chưa có/khác tên), 0 lỗi (~2s).
- ✅ **GUI**: nút "Import Excel" + modal (bảng kết quả tô màu theo trạng thái ok/warn/skip/loi) ở `GUI/DM_NhanVien`; action `import` ajax_handler (multipart, whitelist `.xlsx` ≤10MB, `set_time_limit(120)`). Form thêm/sửa NV bổ sung khu vực **CCHN** (7 trường mới), `getById`/insert/update đã map. DTO/DAL selectSql/insert/update cập nhật.
...
- ✅ **Danh sách HV**: subquery `so_ghi_danh` → HV chưa ghi danh CTĐT nào tô **vàng nhạt** + tag "chưa ghi danh" (cả table & card view, không áp dụng thùng rác). Thêm **filter theo ngày tạo** (từ–đến, bao trọn 2 mốc) áp dụng cả list & export.

### 2026-07-16 — Module Theo dõi tín chỉ CME (cập nhật kiến thức y khoa liên tục)
- ✅ **3 bảng mới** (`docs/sql_cme.sql` — user tự chạy): `dt_cme_nhom` (5 nhóm hình thức), `dt_cme_loai` (~20 loại + công thức quy đổi), `dt_cme_ghi_nhan` (sổ ghi nhận hoạt động/NV). SQL cũng khai báo 2 form `DT_CME` + `DT_CME_DanhMuc` vào `dm_danh_sach_form`.
- ✅ **3 kiểu quy đổi** (`dt_cme_loai.kieu_quy_doi`): `theo_tiet` (số tiết × hệ số), `co_dinh` (số lượng × giờ/đơn vị), `theo_nam` (khoán giờ/năm, bỏ qua số lượng). `DT_Cme_BUS::tinhGioTinChi()` là nơi tính; `gio_tin_chi` **snapshot khi lưu** (không tính động qua danh mục → không lệch khi đổi chính sách).
- ✅ **Seed** `seed_cme.php` (chạy: `php seed_cme.php`, idempotent theo `ma_nhom`/`ma_loai`): nạp 5 nhóm + 24 loại từ Bảng quy đổi kèm Thông báo + 2 key cấu hình ngưỡng **`CME_NGUONG_GIO=24`, `CME_CHU_KY_NAM=1`** (24 giờ/năm — chốt với user) vào `DM_CAU_HINH`. Sửa ngưỡng tại **Hệ thống → Cấu hình → nhóm "Đào tạo y khoa liên tục (CME)"**.
- ✅ **BUS/DAL/Entities**: `DT_Cme_BUS` (nhóm/loại/ghi nhận + `soTheoDoiNhanVien` tính tổng giờ chu kỳ + % đạt ngưỡng + tách nhóm + `baoCaoTheoNhanVien`). Ngưỡng đọc từ `DM_CAU_HINH` (cấu hình được, mặc định 48 giờ/2 năm).
- ✅ **GUI Danh mục quy đổi** (`GUI/DT_CME_DanhMuc`): 2 tab Nhóm & Loại, form loại có gợi ý công thức theo kiểu.
- ✅ **GUI Sổ ghi nhận** (`GUI/DT_CME`): list + stat (tổng giờ/NV/bản ghi theo năm), lọc theo năm/khoa/nhóm/search, form ghi nhận **combo 2 cấp nhóm→loại + ô "giờ tín chỉ tự tính" realtime** (action `tinhThu`), **drawer sổ theo dõi/NV** (thanh tiến độ % đạt ngưỡng, tách nhóm, chi tiết hoạt động), export Excel.
- ✅ Không duyệt (nhập là tính luôn), không đính kèm minh chứng, đối tượng = nhân viên (`dm_nhan_vien`) — theo chốt với user. Admin (`la_admin=1`) full quyền sẵn.
- ✅ **Mục lớn riêng trên sidebar** "Đào tạo y khoa liên tục" gồm: **Theo dõi tín chỉ** (`GUI/DT_CME/index.php`) + **Danh mục quy đổi** (`GUI/DT_CME_DanhMuc`). Còn **Tổng quan** (`GUI/DT_CME/tong_quan.php` — KPI + bar theo nhóm + top 10 NV + bảng theo khoa) và **Báo cáo đào tạo YKLT** (`GUI/DT_CME/bao_cao.php` — 3 tab theo NV/nhóm/khoa, lọc năm+khoa, export `bao_cao_export.php`) nằm ở **nhóm "Tổng quan" trên cùng** cạnh các dashboard NCKH/Đào tạo. Tất cả dùng quyền `DT_CME`; Danh mục dùng `DT_CME_DanhMuc`. DAL thêm `tongToanVienTheoNhom/tongTheoKhoaPhong/topNhanVien`; BUS thêm `tongQuan($nam)`.

### 2026-07-08 — In phiếu cho học viên (mẫu HTML)
- ✅ **Folder `mps/`** chứa mẫu phiếu in dạng **HTML** với placeholder `{{key}}` + `mps/manifest.php` khai báo danh sách mẫu (`key/ten/file/mo_ta`). Mẫu đầu tiên: `phieu_nhan_xet_thuc_hanh.html` (Phiếu nhận xét quá trình thực hành KBCB — giữ đúng định dạng Word: căn giữa, in đậm/nghiêng, chú thích chân trang, `@page A4`). Thêm mẫu mới = tạo file `.html` + 1 dòng trong manifest.
- ✅ **`DT_PhieuIn_BUS`**: `getTemplates()` (lọc mẫu có file thật), `render($key,$hvId,$khctId)` — đọc HTML, build map dữ liệu từ HV + ghi danh được chọn (thời gian/địa điểm/chuyên khoa lấy từ `dt_khoa_hoc_chuong_trinh`), escape rồi thay `{{key}}`. Ô không có dữ liệu (người hướng dẫn, số CCHN…) để trống điền tay. Chống path traversal qua `basename()` + chỉ nhận mẫu trong manifest.
- ✅ **GUI**: nút **In** (icon `printer`) ở mỗi HV (table + card) → modal chọn mẫu; nếu HV có ghi danh thì có combo chọn CTĐT để điền phần thực hành. Endpoint `GUI/DM_HocVien/print_phieu.php` (GET, quyền XEM) xuất HTML đã điền + auto `window.print()`. Actions dùng chung quyền `DM_HocVien`.
- ✅ **Mẫu "Thẻ học viên"** (`mps/the_hoc_vien.html`): thẻ khổ ngang 90×56mm, dải tiêu đề xanh-hồng theo brand BV, có ảnh (`{{avatar_html}}` — `<img>` nếu có avatar, không thì khối chữ cái đầu đã escape), mã HV, đối tượng, ngày sinh/giới tính/CCCD/đơn vị, ngày cấp. `buildData` thêm key `avatar_url` (qua `AppConfig::baseUrl`) + `avatar_html` (dựng sau vòng escape để giữ HTML). Thêm mẫu chỉ cần file HTML + 1 dòng manifest — GUI tự liệt kê.
- 🐞 **Fix**: `DT_HocVienLop_DAL::getByHocVien` thiếu `khct.dia_diem` → phiếu KCB luôn trống ô "Địa điểm thực hành". Đã bổ sung cột. (Rà soát: render 2 mẫu 0 placeholder sót, mẫu/HV sai báo lỗi đúng, escape initials.)

### 2026-07-16 — Cập nhật danh mục Khoa/Phòng theo danh sách chốt 16.7.2026
- ✅ **`docs/sql_cap_nhat_khoa_phong.sql`** (user tự chạy, sinh từ `docs/danh mục khoa phòng trung tâm 16.7.2026.xlsx`): **cập nhật 49** khoa (mã+tên) · **thêm mới 10** · **xóa mềm 26** (đúng convention, không DELETE) → danh mục còn **đúng 59 khoa khớp 100% Excel**.
- ✅ **Thứ tự bước bắt buộc**: (1) thêm khoa mới → (2) **remap 4 CTĐT** khỏi khoa sắp xóa → (3) đổi mã+tên → (4) xóa mềm → (5) gán khoa cho NV import. Remap: `Khoa Nội Tiết - ĐTĐ`→`Nội Tiết`, `Trung Tâm Tim Mạch`→`Nội tim mạch 1`, `Khoa Nội Huyết Học Lâm Sàng`→`Huyết học lâm sàng` (`dt_chuong_trinh.khoa_phong_id` có **FK thật** → không remap trước sẽ lỗi FK).
- 🐞 **2 bug đã bắt khi test trên dev** (nhờ chạy thật + khôi phục backup): (a) key `$remap` viết không dấu nên không khớp `norm()` giữ dấu → phần remap **rỗng**; (b) remap trỏ khoa đích **theo mã** trong khi mã chỉ đổi ở bước sau → 2 CTĐT bị set **NULL**. Fix: dùng **id cứng** cho khoa đã có, tra theo `ten_khoa` cho khoa vừa thêm.
- ✅ **BƯỚC 5**: 10 lệnh gán khoa cho **121/124 NV** import trước đây chưa map được (các khoa đó giờ đã có: Ngoại tổng hợp 1/2, Nội Tiết, Huyết học lâm sàng, Vi rút-Ký sinh trùng…). NV gắn khoa: **1311 → 1432**; còn 3 NV tên khoa lạ (Chuyên gia, Z.Dị Ứng Hô Hấp, z.Hồi sức tích cực nội khoa) gán tay.
- ✅ Kiểm chứng trên dev: 0 NV/CTĐT trỏ khoa đã xóa · 4/4 CTĐT remap đúng · 0 mã trùng · danh mục khớp Excel (0 thiếu/thừa/lệch tên) · SQL idempotent.

### 2026-07-16 — CME: màn Cảnh báo nhân viên chưa đạt ngưỡng
- ✅ **`GUI/DT_CME/canh_bao.php`** (menu **Đào tạo y khoa liên tục → Cảnh báo chưa đạt**, quyền `DT_CME`): liệt kê NV **chưa đạt** ngưỡng giờ tín chỉ theo cấu hình `CME_NGUONG_GIO`/`CME_CHU_KY_NAM`. 4 KPI (tổng NV đang làm / đã đạt / chưa đạt / **chưa ghi nhận hoạt động nào**), thanh tiến độ + % + số giờ còn thiếu, lọc theo **năm / khoa-phòng / tìm tên-mã**, export `canh_bao_export.php`. Dòng NV 0 giờ tô đỏ nhạt + tag "chưa ghi nhận"; sắp xếp ít giờ nhất lên đầu.
- ✅ **`DT_CmeGhiNhan_DAL::nhanVienChuaDat()`** — **bắt đầu từ `dm_nhan_vien` + LEFT JOIN** ghi nhận (điều kiện năm đặt ở `ON` chứ không phải `WHERE`) nên **bao gồm cả NV chưa có ghi nhận nào** (nếu INNER JOIN sẽ bỏ sót đúng nhóm cần cảnh báo nhất). `HAVING SUM(gio) < :nguong`. Kèm `thongKeCanhBao()` + `DT_Cme_BUS::canhBaoChuaDat($nam,$opts,$page,$size)` (tính `con_thieu`, `phan_tram`). Chỉ tính NV `trang_thai=1` (đang làm).
- ✅ **Phân trang + lọc trạng thái**: `nhanVienChuaDat` nhận `$page`/`$pageSize` (**`$page=0` = lấy tất cả**, dùng cho export); đếm tổng bằng `SELECT COUNT(*) FROM (…) t` (bọc subquery vì có GROUP BY+HAVING). Lọc trạng thái áp ở **HAVING**: `chua_ghi_nhan` (=0 giờ) · `thieu_nhieu` (0 < giờ < 50% ngưỡng) · `sap_dat` (≥ 50% ngưỡng). Test: 1329+52+29 = 1410 khớp tổng; export theo đúng filter đang chọn.

### 2026-07-16 — CME: xem chi tiết bản ghi + đính kèm nhanh ở cột Hành động
- ✅ **Cột Hành động** (Theo dõi tín chỉ) giờ có 4 nút: **👁 Xem chi tiết** · **📎 Đính kèm** (tô xanh khi đã có minh chứng, tooltip hiện tên file) · ✏️ Sửa · 🗑 Xóa.
- ✅ **Drawer chi tiết** (`openChiTiet`): hero giờ tín chỉ + tên NV/khoa; nhóm **Hoạt động** (tên, nhóm/loại hình thức, vai trò, số lượng kèm đơn vị, kiểu quy đổi, giờ TC, năm, thời gian), nhóm **Nhân viên**, **Minh chứng** (xem/tải, hoặc link "Đính kèm ngay" nếu chưa có), **Ghi chú**, dòng audit (người tạo/cập nhật + thời điểm); nút "Sổ theo dõi của NV" và "Sửa".
- ✅ **Đính kèm nhanh** (`capNhatMinhChung` + `DT_CmeGhiNhan_DAL::updateMinhChung`): modal gọn chỉ xử lý file — **chỉ ghi 3 cột minh chứng, KHÔNG tính lại giờ tín chỉ / không đụng trường nghiệp vụ**. Thay file → xóa file cũ *sau khi* ghi DB thành công. Thêm icon `paperclip` vào IconHelper.

### 2026-07-16 — CME: đính kèm minh chứng (chứng chỉ PDF)
- ✅ **3 cột mới** `dt_cme_ghi_nhan` (SQL user tự chạy: `docs/sql_cme_minh_chung.sql`): `minh_chung` (tên file trên server), `minh_chung_goc` (tên gốc), `minh_chung_size`.
- ✅ **Upload**: `DT_Cme_BUS::uploadMinhChung()` — whitelist **pdf/jpg/jpeg/png** ≤10MB, **kiểm tra magic bytes** (`%PDF-` cho PDF, `getimagesize` cho ảnh) nên đổi đuôi file độc hại thành `.pdf` vẫn bị chặn; tên file random lưu ở `assets/uploads/cme/` (đã có `.htaccess` chặn thực thi PHP toàn cây uploads).
- ✅ **Quy ước update** (giống avatar HV): `minh_chung = null` giữ file cũ · `''` gỡ file · có giá trị = thay file (tự xóa file cũ). Upload xong mà lưu DB lỗi → tự dọn file rác.
- ✅ **GUI**: form ghi nhận thêm ô đính kèm (hiện file hiện tại + nút gỡ, form chuyển sang `FormData`/multipart); **icon file** ở cột Hoạt động (list) và trong drawer Sổ theo dõi → mở xem. Endpoint `GUI/DT_CME/tai_minh_chung.php` (check quyền XEM, `basename()` chống path traversal, `inline` để xem PDF, `?tai=1` để tải, `X-Content-Type-Options: nosniff`). Export thêm cột **Minh chứng**.

### 2026-07-16 — Nhân viên: xem chi tiết + export đủ cột; 🐞 fix export bị cắt còn 500 dòng
- 🐞 **Fix nghiêm trọng — MỌI export chỉ xuất 500 dòng**: `PaginationHelper::normalize()` kẹp `pageSize` ở 500 (đúng, để chặn pageSize khổng lồ từ POST), nhưng 17 file `export.php` gọi `getPaged(1, 100000, ...)` nên **âm thầm mất dữ liệu** (VD nhân viên xuất 500/1435). Fix: thêm **`ExportHelper::fetchAll($fetcher)`** (`PUBLIC/Common/ExportHelper.php`, nạp ở bootstrap) — lặp từng trang 500 rồi gộp; **giữ nguyên trần 500 cho AJAX** nên không hở bảo mật. Đã áp cho toàn bộ 17 export. Test: 18/18 export chạy OK, nhân viên xuất đủ 1435/1435.
- ✅ **Xem thông tin nhân viên**: click tên NV (hoặc icon 👁) → drawer hiển thị **toàn bộ** thông tin theo 3 nhóm: cơ bản, **CCHN** (số/ngày cấp/phạm vi/QĐ bổ sung/điều chỉnh/chuyên khoa cập nhật), liên hệ; có nút "Sửa thông tin". Cột Khoa ở list: nếu chưa map danh mục thì hiện **tên khoa gốc từ file import** (tag vàng) thay vì "-".
- ✅ **Export nhân viên theo đúng cấu trúc file import** (13 cột đầu trùng mẫu "Danh sách người hành nghề toàn viện": TT, MNV, Họ tên, K/P/TT, Văn bằng, Ngày sinh, Phạm vi hành nghề, Số CCHN, Ngày cấp CCHN, QĐ bổ sung, Điều chỉnh phạm vi, Ngày điều chỉnh, Chuyên khoa cập nhật) + 6 cột quản lý (giới tính, chức danh, điện thoại, email, địa chỉ, trạng thái). Khoa lấy `ten_khoa_phong`, fallback `khoa_phong_text`.

### 2026-07-16 — Tách quyền dashboard Đào tạo + Excel export đẹp hơn
- ✅ **Tách quyền**: mục "Đào tạo" (nhóm Tổng quan) đổi từ module key `Dashboard` → **`DT_Dashboard`** (form mới) để nhóm chỉ có quyền CME không còn thấy mục này. SQL user tự chạy: `docs/sql_tach_quyen_dashboard_dao_tao.sql` (tạo form + tự cấp quyền cho các nhóm đang có `DT_BaoCao`). Nhớ xóa cache phân quyền sau khi chạy.
- ✅ **`ExcelHelper` nâng cấp style** (áp dụng cho **toàn bộ** export của hệ thống): **auto-resize bề rộng cột** theo nội dung (kẹp 8–48), **header xanh lá chữ trắng** + canh giữa + wrap, **viền mảnh** mọi ô, **zebra** dòng chẵn/lẻ, **đóng băng dòng header** + **AutoFilter**, tiêu đề lớn gộp ô. Tự nhận diện **dòng tổng**: ô bắt đầu bằng "TỔNG CỘNG" → nền vàng đậm, "TỔNG ..." → nền xanh nhạt. Hằng số style: `S_HEADER/S_TITLE/S_CELL/S_CELL_ALT/S_TOTAL/S_SUBTOTAL`.
